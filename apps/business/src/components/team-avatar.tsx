...
import { stripSpecialCharacters } from "@loopearn/utils";
...
export function TeamAvatar({ teamId, logoUrl, name }) {
  ...
  const handleUpload = async (evt: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (url) {
      action.execute({ avatar_url: url, revalidatePath: "/settings" });
    }
  };
      ...
      <div className="flex justify-between items-center pr-6">
        ...
        >
          {isLoading ? (
            ...
            <>
              <AvatarImage src={logoUrl} alt={name} width={64} height={64} />
              <AvatarFallback>
